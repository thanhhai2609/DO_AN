...
function addProductFromSingle(action, product_id, stock) {
    ...
    if (stock > 0) {
        ...
        // Cập nhật thông báo hoặc giao diện người dùng nếu cần
        document.getElementById("stock-display").textContent = `Số lượng còn lại: ${stock}`;
    } else {
        document.getElementById("error-stock").textContent = "Không còn hàng trong kho!";
    }
    $.ajax({
        url: "/PRJ301_Sp24_ClothesShop/CartServlet",
        type: "get",
...
function updateProductCart(action, product_id, stock) {
    ...
    console.log(stock, sl);
    if (stock < sl) {
        document.getElementById("error").textContent = "Hết hàng!";
        return;
    }
    $.ajax({
        url: "/PRJ301_Sp24_ClothesShop/CartServlet",
